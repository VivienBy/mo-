    ...
    <div id="commentsList">
        ...
        if($result) {
            ...
            for($i = 0; $i < $rows; ++$i) {
                ...
                echo "<tr>";
                for($j = 1; $j < 4; ++$j) {
                    if($j == 3) echo "$row[$j]</td>"; else if($j == 2) echo "<td align='left'>$row[$j]    <hr/><form action ='sendlike.php?id=".$_GET["id"]."' method='post'><input class='like' name='likes$i' type='submit' value = '❤'/></form>"; else echo "<td>$row[$j]</td>";; /* if (!isset($_POST['likes$i'])) die('Не передано имя');*/
